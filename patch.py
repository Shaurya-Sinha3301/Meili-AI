...
def generate_benchmark_report(analyzer: BenchmarkAnalyzer, output_dir: str):
    ...
    with sqlite3.connect(analyzer.db_path) as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT AVG(runtime_seconds / MAX(1, activities_scheduled)) FROM optimization_metrics WHERE activities_scheduled > 0")
    ...
    format_metric("Preference Quality", ["satisfaction_score", "activities_scheduled", "activities_skipped", "accessibility_score", "fatigue_score", "preference_coverage_pct", "time_window_compliance"])
...
def generate_resume_evidence(db_path: str, output_path: str = None) -> bool:
    ...
    if "runtime_seconds" in stats:
        ...
        add_evidence(
            ...
            "SELECT runtime_seconds FROM optimization_metrics ORDER BY runtime_seconds ASC LIMIT 1 OFFSET (COUNT(*)/2)",
            "optimization_metrics",
        ...
        add_evidence(
            "P99 latency",
            f"{rt['p99']*1000:.1f} ms",
            "SELECT runtime_seconds FROM optimization_metrics ORDER BY runtime_seconds ASC LIMIT 1 OFFSET (COUNT(*)*0.99)",
            "optimization_metrics"
        )
...
with open(r'd:\Projects\Merydian\backend\scripts\benchmark\report_generator.py', 'w', encoding='utf-8') as f:
    f.write(new_code)
print("Rewrote report_generator.py successfully.")
